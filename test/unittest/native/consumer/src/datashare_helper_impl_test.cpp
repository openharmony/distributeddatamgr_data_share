...
class MockDataShareConnection : public DataShareConnectionBase {
public:
    MOCK_METHOD((std::shared_ptr<DataShareProxy>), GetDataShareProxy,
        (const Uri &uri, const sptr<IRemoteObject> &token), (override));
    MOCK_METHOD(void, UpdateObserverExtsProviderMap,
        (const Uri &uri, const sptr<AAFwk::IDataAbilityObserver> &dataObserver, bool isDescendants), (override));
    MOCK_METHOD(void, DeleteObserverExtsProviderMap,
...
/**
 * @tc.name: DataShareHelperImpl_Release_Test_001
 * @tc.desc: Verify Release() skips Close() and returns true when connection_ is nullptr (silent-access path).
 * @tc.type: FUNC
 * @tc.require: None
 * @tc.precon:
     1. A DataShareHelperImpl instance can be created via the silent-access constructor.
     2. The connection_ member is nullptr by default for the silent-access instance.
     3. The persistentDataCtl_/publishedDataCtl_ members can be cleared to avoid destructor side effects.
 * @tc.step:
     1. Create a silent-access DataShareHelperImpl instance and clear persistent/published controllers.
     2. Confirm connection_ is nullptr.
     3. Call Release() and check the return value.
     4. Check that connection_ is still nullptr after Release().
 * @tc.expect:
     1. Release() returns true.
     2. connection_ remains nullptr (Close() is not invoked).
 */
HWTEST_F(DataShareHelperImplTest, DataShareHelperImpl_Release_Test_001, TestSize.Level0)
{
    LOG_INFO("DataShareHelperImpl_Release_Test_001::Start");
    auto instance = std::make_shared<DataShareHelperImpl>("datashare://datasharehelperimpl.release");
    ASSERT_NE(instance, nullptr);
    instance->persistentDataCtl_ = nullptr;
    instance->publishedDataCtl_ = nullptr;
    EXPECT_EQ(instance->connection_, nullptr);
    bool result = instance->Release();
    EXPECT_TRUE(result);
    EXPECT_EQ(instance->connection_, nullptr);
    LOG_INFO("DataShareHelperImpl_Release_Test_001::End");
}

/**
 * @tc.name: DataShareHelperImpl_Release_Test_002
 * @tc.desc: Verify Release() invokes Close() on the connection and clears connection_/generalCtl_ when
 *           connection_ is not nullptr.
 * @tc.type: FUNC
 * @tc.require: None
 * @tc.precon:
     1. A MockDataShareConnection (DataShareConnectionBase) can be instantiated.
     2. A silent-access DataShareHelperImpl instance can be created and its connection_ member can be assigned.
     3. The persistentDataCtl_/publishedDataCtl_ members can be cleared to avoid destructor side effects.
 * @tc.step:
     1. Create a silent-access DataShareHelperImpl instance and clear persistent/published controllers.
     2. Assign a MockDataShareConnection to connection_ and expect Close() to be called exactly once.
     3. Call Release() and check the return value.
     4. Check that connection_ and generalCtl_ are nullptr after Release().
 * @tc.expect:
     1. Release() returns true.
     2. Close() is called exactly once on the connection.
     3. connection_ and generalCtl_ are nullptr after Release().
 */
HWTEST_F(DataShareHelperImplTest, DataShareHelperImpl_Release_Test_002, TestSize.Level0)
{
    LOG_INFO("DataShareHelperImpl_Release_Test_002::Start");
    auto instance = std::make_shared<DataShareHelperImpl>("datashare://datasharehelperimpl.release");
    ASSERT_NE(instance, nullptr);
    instance->persistentDataCtl_ = nullptr;
    instance->publishedDataCtl_ = nullptr;
    auto mockConnection = std::make_shared<MockDataShareConnection>();
    ASSERT_NE(mockConnection, nullptr);
    instance->connection_ = mockConnection;
    EXPECT_CALL(*mockConnection, Close()).Times(1);
    bool result = instance->Release();
    EXPECT_TRUE(result);
    EXPECT_EQ(instance->connection_, nullptr);
    EXPECT_EQ(instance->generalCtl_, nullptr);
    LOG_INFO("DataShareHelperImpl_Release_Test_002::End");
}

/**
 * @tc.name: DataShareHelperImpl_Destructor_Close_Test_001
 * @tc.desc: Verify the destructor calls Close() on connection_ as a fallback when Release() was not called.
 * @tc.type: FUNC
 * @tc.require: None
 * @tc.precon:
     1. A MockDataShareConnection (DataShareConnectionBase) can be instantiated.
     ...
     3. The persistentDataCtl_/publishedDataCtl_ members can be cleared to avoid destructor side effects.
 * @tc.step:
     1. Create a MockDataShareConnection and expect Close() to be called exactly once.
     2. In a scope block, create a silent-access DataShareHelperImpl instance and clear persistent/published
        controllers.
     3. Assign the MockDataShareConnection to connection_.
     4. Do not call Release(); let the instance be destroyed at the end of the scope block.
 * @tc.expect:
     1. Close() is called exactly once on the connection when the destructor runs.
 */
HWTEST_F(DataShareHelperImplTest, DataShareHelperImpl_Destructor_Close_Test_001, TestSize.Level0)
{
    LOG_INFO("DataShareHelperImpl_Destructor_Close_Test_001::Start");
    auto mockConnection = std::make_shared<MockDataShareConnection>();
    ASSERT_NE(mockConnection, nullptr);
    EXPECT_CALL(*mockConnection, Close()).Times(1);
    {
        auto instance = std::make_shared<DataShareHelperImpl>("datashare://datasharehelperimpl.dtor");
        ASSERT_NE(instance, nullptr);
        instance->persistentDataCtl_ = nullptr;
        instance->publishedDataCtl_ = nullptr;
        instance->connection_ = mockConnection;
        EXPECT_NE(instance->connection_, nullptr);
        // Release() is not called; the instance is destroyed at the end of this scope.
    }
    LOG_INFO("DataShareHelperImpl_Destructor_Close_Test_001::End");
}

/**
 * @tc.name: DataShareHelperImpl_Destructor_NoClose_Test_002
 * @tc.desc: Verify the destructor does not call Close() and does not crash when connection_ is nullptr
 *           (Release() was called or silent access).
 * @tc.type: FUNC
 * @tc.require: None
 * @tc.precon:
     1. A silent-access DataShareHelperImpl instance can be created.
     2. The persistentDataCtl_/publishedDataCtl_ members can be cleared to avoid destructor side effects.
 * @tc.step:
     1. In a scope block, create a silent-access DataShareHelperImpl instance and clear persistent/published
        controllers.
     2. Confirm connection_ is nullptr.
     3. Do not call Release(); let the instance be destroyed at the end of the scope block.
 * @tc.expect:
     1. The destructor completes without crashing.
     2. The Close() fallback branch is not taken because connection_ is nullptr.
 */
HWTEST_F(DataShareHelperImplTest, DataShareHelperImpl_Destructor_NoClose_Test_002, TestSize.Level0)
{
    LOG_INFO("DataShareHelperImpl_Destructor_NoClose_Test_002::Start");
    {
        auto instance = std::make_shared<DataShareHelperImpl>("datashare://datasharehelperimpl.noclose");
        ASSERT_NE(instance, nullptr);
        instance->persistentDataCtl_ = nullptr;
        instance->publishedDataCtl_ = nullptr;
        EXPECT_EQ(instance->connection_, nullptr);
        // Release() is not called; connection_ is nullptr, so the destructor must not call Close().
    }
    LOG_INFO("DataShareHelperImpl_Destructor_NoClose_Test_002::End");
}
} // namespace DataShare
} // namespace OHOS