...
public:
    ...
    template <typename... Names>
    Expected<ani_class, ani_status> FindClass(const char* firstNs, const char* secondNs, Names... restNs,
        const char* clsName)
    {
...
class AniObjectFactory {
public:
    AniObjectFactory(ani_env *env)
        : env_(env)
    {
    }
    ...
    {
        auto cls = AniTypeFinder(env_).FindClass(clsName);
        if (!cls.has_value()) {
            return cls.error();
        }
    ...
    {
        auto cls = AniTypeFinder(env_).FindClass(nsName, restNs..., clsName);
        if (!cls.has_value()) {
            return cls.error();
        }
