...
template<typename T>
using convertCallback = bool(*)(ani_env*, ani_ref&, ani_ref&, std::shared_ptr<T>);

template<typename F, typename T>
static bool forEachMapEntry(ani_env *env, ani_object map_object, F &&callback, std::shared_ptr<T> records)
{
    ani_ref keys;
    if (ANI_OK !=
    ...
    while (success) {
        ...
        if (ANI_OK != env->Object_CallMethodByName_Ref(map_object, "$_get", nullptr,
            &value_obj, key_value)) {
            std::cout << "Failed to get value for key" << std::endl;
            success = false;
            break;
        }

        if (!callback(env, key_value, value_obj, records)) {
            success = false;
            break;
        }
    }
