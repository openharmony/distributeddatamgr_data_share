...
std::vector<std::string> DataShareProxy::GetFileTypes(const Uri &uri, const std::string &mimeTypeFilter)
{
    ...
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_GET_FILE_TYPES), data, reply, option);
...
int DataShareProxy::OpenFile(const Uri &uri, const std::string &mode)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_OPEN_FILE), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
...
int DataShareProxy::OpenRawFile(const Uri &uri, const std::string &mode)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_OPEN_RAW_FILE), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
...
int DataShareProxy::Insert(const Uri &uri, const DataShareValuesBucket &value)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_INSERT), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
...
int DataShareProxy::Update(const Uri &uri, const DataSharePredicates &predicates, const DataShareValuesBucket &value)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_UPDATE), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
...
int DataShareProxy::Delete(const Uri &uri, const DataSharePredicates &predicates)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_DELETE), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
    ...
    std::vector<std::string> &columns, DatashareBusinessError &businessError)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_QUERY), data, reply, option);
...
std::string DataShareProxy::GetType(const Uri &uri)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_GET_TYPE), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
        LOG_ERROR("GetFileTypes fail to SendRequest. err: %{public}d", err);
        return type;
    }
...
int DataShareProxy::BatchInsert(const Uri &uri, const std::vector<DataShareValuesBucket> &values)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_BATCH_INSERT), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
...
bool DataShareProxy::RegisterObserver(const Uri &uri, const sptr<AAFwk::IDataAbilityObserver> &dataObserver)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t result = Remote()->SendRequest(static_cast<uint32_t>(CMD_REGISTER_OBSERVER), data, reply, option);
    if (result == ERR_NONE) {
...
bool DataShareProxy::UnregisterObserver(const Uri &uri, const sptr<AAFwk::IDataAbilityObserver> &dataObserver)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t result = Remote()->SendRequest(static_cast<uint32_t>(CMD_UNREGISTER_OBSERVER), data, reply, option);
    if (result == ERR_NONE) {
        LOG_INFO("SendRequest ok, retval is %{public}d", reply.ReadInt32());
    } else {
        LOG_ERROR("SendRequest error, result=%{public}d", result);
...
bool DataShareProxy::NotifyChange(const Uri &uri)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t result = Remote()->SendRequest(static_cast<uint32_t>(CMD_NOTIFY_CHANGE), data, reply, option);
    if (result == ERR_NONE) {
        LOG_INFO("SendRequest ok, retval is %{public}d", reply.ReadInt32());
    } else {
        LOG_ERROR("SendRequest error, result=%{public}d", result);
...
Uri DataShareProxy::NormalizeUri(const Uri &uri)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_NORMALIZE_URI), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
...
Uri DataShareProxy::DenormalizeUri(const Uri &uri)
{
    ...
    MessageParcel reply;
    MessageOption option;
    int32_t err = Remote()->SendRequest(static_cast<uint32_t>(CMD_DENORMALIZE_URI), data, reply, option);
    if (err != DATA_SHARE_NO_ERROR) {
