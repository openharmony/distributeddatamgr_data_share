...
public:
     ...
     * @return Returns the index of the inserted data record.
     */
    virtual int InsertExt(const Uri &uri, const DataShareValuesBucket &value, std::string &result);
     ...
     * @return Return true if success. otherwise return false.
     */
    virtual bool NotifyChangeWithUser(const Uri &uri, int32_t userId, uint32_t callingToken, int32_t callingPid);
    ...
    virtual void InitResult(std::shared_ptr<ResultWrap> result);
