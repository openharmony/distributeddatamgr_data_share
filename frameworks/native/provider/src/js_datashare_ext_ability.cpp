...
bool JsDataShareExtAbility::RegisterObserverExt(const Uri &uri, const sptr<AAFwk::IDataAbilityObserver> &dataObserver,
    bool isDescendants)
{
    DataShareExtAbility::RegisterObserverExt(uri, dataObserver, isDescendants);
    auto obsMgrClient = DataObsMgrClient::GetInstance();
    if (obsMgrClient == nullptr) {
        LOG_ERROR("obsMgrClient is nullptr");
        return false;
    ...
    if (ret != ERR_OK) {
        LOG_ERROR("obsMgrClient->RegisterObserverExt error return %{public}d", ret);
        return false;
    }
    return true;
}

bool JsDataShareExtAbility::UnregisterObserverExt(const Uri &uri, const sptr<AAFwk::IDataAbilityObserver> &dataObserver)
{
    DataShareExtAbility::UnregisterObserverExt(uri, dataObserver);
    auto obsMgrClient = DataObsMgrClient::GetInstance();
    if (obsMgrClient == nullptr) {
        LOG_ERROR("obsMgrClient is nullptr");
        return false;
    ...
    if (ret != ERR_OK) {
        LOG_ERROR("obsMgrClient->UnregisterObserverExt error return %{public}d", ret);
        return false;
    }
    return true;
}

bool JsDataShareExtAbility::UnregisterObserverExt(const sptr<AAFwk::IDataAbilityObserver> &dataObserver)
{
    DataShareExtAbility::UnregisterObserverExt(dataObserver);
    auto obsMgrClient = DataObsMgrClient::GetInstance();
    if (obsMgrClient == nullptr) {
        LOG_ERROR("obsMgrClient is nullptr");
        return false;
...
bool JsDataShareExtAbility::NotifyChangeExt(const ChangeInfo &changeInfo)
{
    DataShareExtAbility::NotifyChangeExt(changeInfo);
    auto obsMgrClient = DataObsMgrClient::GetInstance();
    if (obsMgrClient == nullptr) {
        LOG_ERROR("obsMgrClient is nullptr");
        return false;
    ...
    if (ret != ERR_OK) {
        LOG_ERROR("obsMgrClient->NotifyChangeExt error return %{public}d", ret);
        return false;
    }
    return true;
}
